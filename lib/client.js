...
function Client(opts) {
  ...
  DEFAULT_CIRCUIT_BREAKER_OPTIONS.isFailure = opts.isFailure || DEFAULT_CIRCUIT_BREAKER_OPTIONS.isFailure;
  this.isFailure = DEFAULT_CIRCUIT_BREAKER_OPTIONS.isFailure;
...
Client.prototype._requestWithRetries = function (method, url, opts, cb) {
  ...
  operation.attempt(function (currentAttempts) {
    client._request(method, url, opts, function (err, res, body) {
      var statsName;
      var error = client.isFailure(err, body);

      if (operation.retry(error)) {
        if (client.logger && error) {
          client.logger.warn('Attempt', currentAttempts, error.message);
        }
        return;
      }
